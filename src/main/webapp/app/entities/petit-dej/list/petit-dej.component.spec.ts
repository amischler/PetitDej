import { ComponentFixture, TestBed } from '@angular/core/testing';
import { HttpHeaders, HttpResponse } from '@angular/common/http';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { of } from 'rxjs';

import { PetitDejService } from '../service/petit-dej.service';

import { PetitDejComponent } from './petit-dej.component';

describe('PetitDej Management Component', () => {
  let comp: PetitDejComponent;
  let fixture: ComponentFixture<PetitDejComponent>;
  let service: PetitDejService;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule],
      declarations: [PetitDejComponent],
    })
      .overrideTemplate(PetitDejComponent, '')
      .compileComponents();

    fixture = TestBed.createComponent(PetitDejComponent);
    comp = fixture.componentInstance;
    service = TestBed.inject(PetitDejService);

    const headers = new HttpHeaders();
    jest.spyOn(service, 'query').mockReturnValue(
      of(
        new HttpResponse({
          body: [{ id: 123 }],
          headers,
        })
      )
    );
  });

  it('Should call load all on init', () => {
    // WHEN
    comp.ngOnInit();

    // THEN
    expect(service.query).toHaveBeenCalled();
    expect(comp.petitDejs?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });
});
